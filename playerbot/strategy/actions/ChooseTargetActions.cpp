...
bool AttackAnythingAction::isUseful() 
{
    ...
    if(!target->IsPlayer() && sServerFacade.IsInFront(bot, target, PlayerbotsCompatibility::GetAttackDistance(target, bot) * 1.5f, M_PI_F*0.5f) && target->IsHostileTo(bot) && target->GetLevel() < bot->GetLevel() + 3.0) //Attack before being attacked.
        return true;
