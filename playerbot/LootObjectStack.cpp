...
void LootObject::Refresh(Player* bot, ObjectGuid guid, bool debug)
{
    ...
    if (go && sServerFacade.isSpawned(go) && PlayerbotsCompatibility::IsGameObjectInUse(go))
        return;
...
bool LootObject::IsLootPossible(Player* bot)
{
    ...
    if (guid.IsCreature())
    {
        ...
        if (creature && sServerFacade.GetDeathState(creature) == CORPSE)
        {
            if (skillId != SKILL_SKINNING && creature->loot.isLooted())
                return false;
        }
    }
    ...
    if (guid.IsGameObject())
    {
        ...
        if (go)
        {
            ...
            if (PlayerbotsCompatibility::IsGameObjectInUse(go) || go->GetGoState() == GO_STATE_ACTIVE)
                return false;
        }
    }
