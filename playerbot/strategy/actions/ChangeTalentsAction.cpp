...
bool ChangeTalentsAction::AutoSelectTalents(Player* bot, std::ostringstream* out, BotRoles role)
{
    ...
    if (PlayerbotsCompatibility::CalculateTalentPoints(bot) > 0 || (!specNo && specLink.empty()))
    {
        ...
        {
            if (PlayerbotsCompatibility::CalculateTalentPoints(bot) > 0)
...
TalentSpec* ChangeTalentsAction::GetBestPremadeSpec(Player* bot, int specId)
{
    ...
    {
        if (spec.points >= PlayerbotsCompatibility::CalculateTalentPoints(bot))
            return &spec;
    }
